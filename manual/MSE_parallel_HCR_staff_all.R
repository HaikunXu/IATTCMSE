...
Weight_S <- data.frame("Steepness" = steepness, "Weight_S" =  rep(1 / length(steepness), length(steepness))) # c(0.46, 0.32, 0.22))
...
# i = 1;  HCR = runs[i,1]; OM = runs[i,6]; itrnum = runs[i,4]
# BET_MSE(pdir,sdir,HS,runs[i, 1],runs[i, 6],runs[i, 4],nquarters,Mcycle,n_extra_R,startquarter,endquarter,EM_comp_fleet,dat_name,ctl_name,ss_name,clean = FALSE)
