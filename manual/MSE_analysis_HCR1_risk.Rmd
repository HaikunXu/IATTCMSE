...
output_counts <- Output_all %>%
  group_by(OM) %>%
  summarise(count = length(unique(itr)))
...
ggplot() +
  geom_violin(aes(x = factor(year), y = Value), data = output_all_new) +
  facet_wrap(~Quantity, scales = "free_y", nrow = 2) +
  theme_bw()
...
ggplot() +
  geom_boxplot(aes(x = as.factor(year), y = CPUE, color = OM), data = CPUE_all %>% filter(Year > 2023)) +
  theme_bw()
```
...
Record_all_3 <- left_join(Record_all, output_all_new) %>%
  filter(Quantity == "SBR_d") %>%
  mutate(SB_ratio = SBR_d / Value)

# compare SB ratio
ggplot() +
  geom_boxplot(aes(x = as.factor(Year), y = SB_ratio, fill = OM), data = Record_all_3) +
  ylab("SBR_d_EM / SBR_d_OM") +
  facet_wrap( ~ OM, nrow = 4) +
  theme_bw()
```