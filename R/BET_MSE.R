...
#' @param Bthr the fraction of unfished biomass the threshold reference point refers to
...
#' @param nsteps specifies the number of steps in the MSE period
...
BET_MSE = function(pdir, sdir, HS, HCR, OM, itr, Bthr, Blim, Tmin, nquarters, Mcycle, n_extra_R) { 
...
# setwd(paste0(pdir, HS, HCR, OM, itr))
...
for (istep in 1:nsteps){
  ...
  if(istep == 1) {
    #read EM output file
    em_out <- r4ss::SS_output(paste0(pdir, HS, "EM/"), covar = FALSE, verbose = FALSE, printstats = FALSE)
    ...
    # e)	Find FHCR from the estimated Sbio using the HCR
    if (SBR_d > 0.2) Fadjust <- 1
    else Fadjust <- SBR_d / 0.2
    Fadjust_ts[1] <- Fadjust
    
    #get Fscale for Btarget = 0.3
    Fmult <- IATTCassessment::Fmult(paste0(pdir, HS, "EM/"))
  ...
  # ForecastFile[18] <- 0 # Harvest control rule method
  # ForecastFile[23] <- 1 # First forecast loop with stochastic recruitment
  ...
  SS_plots(replist=om_out, forecastplot=T, uncertainty=F, datplot=T, plot = 3, verbose = FALSE)
  ...
  Catch_new <- rbind(Catch, Catch_projection_new) %>%
    arrange(fleet, year)
  
  ggplot(catch_new %>% filter(year>0, fleet<15)) +
  ...
  catch_new <- rbind(catch, catch_boot) %>%
    arrange(fleet, year)
  ...
  SS_plots(replist=em_out, forecastplot=T, uncertainty=F, datplot=T, plot = 3, verbose = FALSE)
  
  max_gradient_ts[istep] <- em_out$maximum_gradient_component
  if(em_out$maximum_gradient_component > 1) break # large gradient - the model does not converge
  
  # dynamic SBR
  Dynamic_Bzero <- em_out$Dynamic_Bzero
  SBR_d <- Dynamic_Bzero$SSB[nrow(Dynamic_Bzero)] / Dynamic_Bzero$SSB_nofishing[nrow(Dynamic_Bzero)]
  SBR_d_ts[istep + 1] <- SBR_d
  
  # e)	Find FHCR from the estimated Sbio using the HCR
  if (SBR_d > 0.2) Fadjust <- 1
  else Fadjust <- SBR_d / 0.2
  
  if(istep != nstep) Fadjust_ts[istep + 1] <- Fadjust
  
  #get Fscale for Btarget = 0.3
  Fmult <- IATTCassessment::Fmult(dir_EM)
  Fscale <- Fmult * Fadjust

#h)	Check the Fscale with the 10days maximum and re-adjust with Fscale = current opening +- 10 days / current opening
# NewClosure <- 365 - (365 - CurrentClosure)*Fscale*Fadjust
# 
# if((Fscale > 1) & (CurrentClosure - NewClosure > 10)) {
#   CurrentClosure <- CurrentClosure - 10
#   Fscale <- (365 - CurrentClosure) / (365 - (CurrentClosure + 10))
# }
# 
