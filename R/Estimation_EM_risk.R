...
Estimation_EM_risk = function(dir_istep, dir_EM_previous, dir_OM_Boot, Mcycle, dat_name, ctl_name, ss_name, plot = FALSE, include_LF = TRUE) {
  
  for (m in 1:4) {
    # step 1: create a new folder for the EM
    if(m == 1) dir_EM <- paste0(dir_istep, "EM_Fix/")
    if(m == 2) dir_EM <- paste0(dir_istep, "EM_Mrt/")
    if(m == 3) dir_EM <- paste0(dir_istep, "EM_Sel/")
    if(m == 4) dir_EM <- paste0(dir_istep, "EM_Gro/")
    
    dir.create(dir_EM)
    
    # copy files to the new folder
    files = c(
      paste0(dir_EM_previous[m], ss_name),
      paste0(dir_EM_previous[m], "go_nohess.bat"),
      paste0(dir_EM_previous[m], "forecast.ss"),
      paste0(dir_EM_previous[m], "starter.ss")
    )
    file.copy(from = files, to = dir_EM, overwrite = TRUE)
    
    # change data file
    dat_EM_previous <- r4ss::SS_readdat_3.30(file = paste0(dir_EM_previous[m], dat_name), verbose = FALSE)
    ...
    ctl <- r4ss::SS_readctl_3.30(
      file = paste0(dir_EM_previous[m], ctl_name),
      verbose = FALSE,
    ...
    ctl$SR_parms$INIT[1] <- ctl$SR_parms$INIT[1] + 0.1 # adding 0.25 makes the EM more likely to converge
  ...
  return(paste0(dir_istep, c("EM_Fix/", "EM_Mrt/", "EM_Sel/", "EM_Gro/")))
