...
BET_MSE_risk = function(pdir, sdir, HS, HCR, OM, itrnum, nquarters, Mcycle, n_extra_R, startquarter, endquarter, EM_comp_fleet, dat_name, ctl_name, ss_name, clean = FALSE, plot = FALSE) { 
  ...
  for (istep in 1:nsteps){
    ...
    # *************************************************************************************
    if(istep == 1) dir_EM_HCR <- paste0(pdir, HS, c("EM_Fix/", "EM_Mrt/", "EM_Sel/", "EM_Gro/"))
    ...
    if(istep < nsteps) step6 <- IATTCMSE::Estimation_EM_risk(dir_istep, dir_EM_previous, dir_OM_Boot, Mcycle, dat_name, ctl_name, ss_name, plot = plot)
