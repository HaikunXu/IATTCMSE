...
BET_MSE_risk = function(pdir,
  ...
  for (istep in 1:nsteps) {
    ...
    # *************************************************************************************
    if (istep == 1)
      dir_EM_HCR <- paste0(pdir, HS, c("EM_Fix/", "EM_Mrt/", "EM_Sel/", "EM_Gro/"))
    else
      dir_EM_HCR <- paste0(pdir, HS, HCR, OM, itr, "step", istep - 1, "/", c("EM_Fix/", "EM_Mrt/", "EM_Sel/", "EM_Gro/"))
    
    if (HCR == "HCR_staff_risk/")
      step2 <- IATTCMSE::HCR_staff_risk(dir_EM = dir_EM_HCR, istep, CurrentClosure)
    if (HCR == "HCR_staff_0_risk/")
      step2 <- IATTCMSE::HCR_staff_0_risk(dir_EM = dir_EM_HCR, istep, CurrentClosure)
